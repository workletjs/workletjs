...
describe('WolWebGLTileLayerComponent', () => {
  let fixture: ComponentFixture<TestWebGLTileLayerComponent>;
  let testComponent: TestWebGLTileLayerComponent;
  let mapInstance: Map;
  let layerComponent: WolWebGLTileLayerComponent;
  let layer: WebGLTileLayer;

  beforeEach(async () => {
    fixture = TestBed.createComponent(TestWebGLTileLayerComponent);
    ...
    layer = layerComponent.getInstance() as WebGLTileLayer;
  });
  ...
  it('should call updateStyleVariables when wolVariables changes via ngOnChanges', () => {
    ...
    testComponent.variables.set({ brightness: 0.5 });
    fixture.detectChanges();
    expect(spy).toHaveBeenCalledWith({ brightness: 0.5 });
  });
...
@Component({
  selector: 'wol-test-webgl-tile-layer',
  imports: [WolMapComponent, WolViewComponent, WolWebGLTileLayerComponent],
  template: `
    <wol-map>
...
class TestWebGLTileLayerComponent {
  readonly className = signal('custom-class');
  readonly style = signal<Style | undefined>(undefined);
  readonly variables = signal<Record<string, number> | undefined>(undefined);
  ...
  readonly source = signal<SourceType | undefined>(new OSM());
