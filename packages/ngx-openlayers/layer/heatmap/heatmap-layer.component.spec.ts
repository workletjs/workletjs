...
import { NumberExpression } from 'ol/style/flat';
...
describe('WolHeatmapLayerComponent', () => {
  let fixture: ComponentFixture<TestHeatmapLayerComponent>;
  let testComponent: TestHeatmapLayerComponent;
  let mapInstance: Map;
  let layerComponent: WolHeatmapLayerComponent;
  let layer: Heatmap<Feature<Geometry>>;
  ...
  beforeAll(() => {
    ...
    const mockCtx = {
      ...
      fillRect: vi.fn(),
      getImageData: vi.fn().mockReturnValue({ data: new Uint8ClampedArray(256 * 4) }),
      putImageData: vi.fn(),
  ...
  beforeEach(async () => {
    fixture = TestBed.createComponent(TestHeatmapLayerComponent);
    ...
    layerComponent = fixture.debugElement.query(
      By.directive(WolHeatmapLayerComponent),
    ).componentInstance;
    layer = layerComponent.getInstance() as Heatmap<Feature<Geometry>>;
  });
  ...
  it('should initialize the layer with provided inputs', () => {
    ...
    expect(layer.getGradient()).toEqual(testComponent.gradient());
    expect(layer.getRadius()).toBe(testComponent.radius());
    expect(layer.getBlur()).toBe(testComponent.blur());
  ...
  it('should call setGradient when wolGradient changes via ngOnChanges', () => {
    const spy = vi.spyOn(layer, 'setGradient');
    const newGradient = ['#ff0', '#f00'];
    testComponent.gradient.set(newGradient);
    fixture.detectChanges();
    expect(spy).toHaveBeenCalledWith(newGradient);
    expect(layer.getGradient()).toEqual(newGradient);
  });

  it('should call setRadius when wolRadius changes via ngOnChanges', () => {
    const spy = vi.spyOn(layer, 'setRadius');
    testComponent.radius.set(20);
    fixture.detectChanges();
    expect(spy).toHaveBeenCalledWith(20);
    expect(layer.getRadius()).toBe(20);
  });

  it('should call setBlur when wolBlur changes via ngOnChanges', () => {
    const spy = vi.spyOn(layer, 'setBlur');
    testComponent.blur.set(20);
    fixture.detectChanges();
    expect(spy).toHaveBeenCalledWith(20);
  ...
  it('should update source via ngOnChanges', () => {
    const newSource = new VectorSource<Feature<Geometry>>();
    testComponent.source.set(newSource);
    fixture.detectChanges();
    expect(layer.getSource()).toBe(newSource);
  });

  it('should call setWeight when wolWeight changes via ngOnChanges', () => {
    const spy = vi.spyOn(layer, 'setWeight');
    const weightFn = (feature: Feature) => Number(feature.get('weight') ?? 1);
    testComponent.weight.set(weightFn);
    fixture.detectChanges();
    expect(spy).toHaveBeenCalledWith(weightFn);
  });

  it('should call updateStyleVariables when wolVariables changes via ngOnChanges', () => {
    const spy = vi.spyOn(layer, 'updateStyleVariables');
    testComponent.variables.set({ intensity: 0.9 });
    fixture.detectChanges();
    expect(spy).toHaveBeenCalledWith({ intensity: 0.9 });
  });
  ...
  it('should update wolGradient model and host signal when OL fires change:gradient', async () => {
    const newGradient = ['#0f0', '#00f'];
    const spy = vi.spyOn(layerComponent.wolGradient, 'set');
    layer.setGradient(newGradient);
    fixture.detectChanges();
    await fixture.whenStable();
    expect(spy).toHaveBeenCalledWith(newGradient);
    expect(testComponent.gradient()).toEqual(newGradient);
  });

  it('should update wolRadius model and host signal when OL fires change:radius', async () => {
    const spy = vi.spyOn(layerComponent.wolRadius, 'set');
    layer.setRadius(25);
    fixture.detectChanges();
    await fixture.whenStable();
    expect(spy).toHaveBeenCalledWith(25);
    expect(testComponent.radius()).toBe(25);
  });

  it('should update wolBlur model and host signal when OL fires change:blur', async () => {
    const spy = vi.spyOn(layerComponent.wolBlur, 'set');
    layer.setBlur(25);
    fixture.detectChanges();
    await fixture.whenStable();
    expect(spy).toHaveBeenCalledWith(25);
    expect(testComponent.blur()).toBe(25);
  });
...
@Component({
  selector: 'wol-test-heatmap-layer',
  imports: [WolMapComponent, WolViewComponent, WolHeatmapLayerComponent],
  template: `
    <wol-map>
...
class TestHeatmapLayerComponent {
  readonly className = signal('custom-class');
  readonly weight = signal<((feature: Feature) => number) | undefined>(undefined);
  readonly variables = signal<Record<string, number | number[] | string | boolean> | undefined>(
    undefined,
  );
  ...
  readonly gradient = signal<string[]>(['#00f', '#0ff', '#0f0', '#ff0', '#f00']);
  readonly radius = signal<NumberExpression>(8);
  readonly blur = signal<NumberExpression>(15);
  readonly source = signal<VectorSource<Feature<Geometry>> | undefined>(
