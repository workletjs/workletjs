...
const DEFAULT_STYLE: FlatStyleLike = { 'fill-color': 'rgba(100,100,200,0.5)' };

describe('WolWebGLVectorLayerComponent', () => {
  let fixture: ComponentFixture<TestWebGLVectorLayerComponent>;
  let testComponent: TestWebGLVectorLayerComponent;
  let mapInstance: Map;
  let layerComponent: WolWebGLVectorLayerComponent;
  let layer: WebGLVectorLayer;

  beforeEach(async () => {
    fixture = TestBed.createComponent(TestWebGLVectorLayerComponent);
    ...
    layerComponent = fixture.debugElement.query(
      By.directive(WolWebGLVectorLayerComponent),
    ).componentInstance;
    layer = layerComponent.getInstance() as WebGLVectorLayer;
  });

  it('should create the layer and register it on the map', () => {
    expect(layerComponent).toBeTruthy();
    expect(layer).toBeInstanceOf(WebGLVectorLayer);
  ...
  it('should call setStyle when wolStyle changes via ngOnChanges', () => {
    ...
    const newStyle: FlatStyleLike = { 'fill-color': 'red' };
  ...
  it('should call updateStyleVariables when wolVariables changes via ngOnChanges', () => {
    ...
    const variables: StyleVariables = { brightness: 0.5 };
    testComponent.variables.set(variables);
    fixture.detectChanges();
...
@Component({
  selector: 'wol-test-webgl-vector-layer',
  imports: [WolMapComponent, WolViewComponent, WolWebGLVectorLayerComponent],
  template: `
    <wol-map>
...
class TestWebGLVectorLayerComponent {
  readonly className = signal('custom-class');
  readonly style = signal<FlatStyleLike>(DEFAULT_STYLE);
  readonly variables = signal<StyleVariables | undefined>(undefined);
