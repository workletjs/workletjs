...
Layer source for map tiles from an OGC API - Tiles service that provides "map" type tiles. The
service must conform to at least the core (http://www.opengis.net/spec/ogcapi-tiles-1/1.0/conf/core)
and tileset (http://www.opengis.net/spec/ogcapi-tiles-1/1.0/conf/tileset) conformance classes. For
supporting the `wolCollections` property, the service must conform to the collections selection
