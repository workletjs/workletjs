...
export interface DataTileSourceHostRef<T extends DataTileSource> {
  setSource(source: T): DisposeRef;
  getInstance(): TileLayer | WebGLTileLayer | FlowLayer | RasterSource | undefined;
}

export function useDataTileSourceHostRef<T extends DataTileSource>(
  sourceName: string,
): DataTileSourceHostRef<T> {
  ...
  const flowLayer = inject(WolFlowLayerComponent, options);
  ...
  if (flowLayer) {
    return {
      setSource: (source) => {
        flowLayer.getInstance()?.setSource(source as DataTileSource);
        return () => {
          flowLayer.getInstance()?.setSource(null);
        };
      },
      getInstance: () => flowLayer.getInstance(),
    };
  }

  throw new Error(
    `No TileSource host found. Please wrap the ${sourceName} component in a RasterSource, ` +
      `WebGLTileLayer, FlowLayer or TileLayer component.`,
  );
}
