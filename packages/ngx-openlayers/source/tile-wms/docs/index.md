...
WMS can be used as a Tile layer, as shown here, or as an Image layer, as shown in the [Single Image WMS](/sources/image-wms#single-image-wms) example. Tiles can be cached, so the browser will not re-fetch data for areas that were viewed already. But there may be problems with repeated labels for WMS servers that are not aware of tiles, in which case single image WMS will produce better cartography.

{{ NgDocActions.demo("WolTiledWMSExampleComponent") }}

### Tiled WMS Wrapping

By default, WMS tiles are reused across the 180° meridian. This behavior can be disabled by setting the `wolWrapX` property to `false`.
